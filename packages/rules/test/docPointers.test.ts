 ...
 *
 * - `rule.v1.json`'s own description said "See docs/rule-authoring.md". **That
 *   file has never existed** - the guide is in `CONTRIBUTING.md`. It is the
 *   first line a non-developer rule author reads, and it sent them nowhere.
 * - `facts.ts` and a rule-verification record both cited
 *   `docs/prd/private-foundation-fact.md`. That path is a **gitignored symlink**
 *   into an internal repo (`.gitignore`: "PRDs live in the INTERNAL stonedog-prd
 *   repo, never here - this repo is PUBLIC"). It resolves on one workstation and
 *   nowhere else.
 *
...
/** Every `docs/...md` reference, with the file and line it appears on. */
